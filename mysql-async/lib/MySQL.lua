...
function MySQL.execute(name, args, cb)
    MySQL.ready(function()
        if MySQLCommands[name] then
            ...
            MySQL.Sync.execute(MySQLCommands[name], args2)
            if cb then
                local task = Task(cb)
                task({})
            end
...
function MySQL.query(name, args, cb)
    if isReady then
        if MySQLCommands[name] then
            ...
            if cb then
                local task = Task(cb)
                local rws = MySQL.Sync.fetchAll(MySQLCommands[name], args2)
                task({rws})
            else
                MySQL.Sync.execute(MySQLCommands[name], args2)
            end
        ...
        print("^1MySQL: error the query "..name.." in resource "..GetCurrentResourceName().." must be placed in MySQL.ready() function")
    end
end
