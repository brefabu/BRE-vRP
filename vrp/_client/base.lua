...
function tvRP.displayHelp(x,y,z,str,radius)--need to be called in while loop
  if Vdist(vRP.getPosition(),x,y,z) < radius then
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(500)
    local ped = GetPlayerPed(-1)
    local proximity = config.voice_proximity
    ...
    if IsPedSittingInAnyVehicle(ped) then
      ...
      if IsThisModelACar(hash) or IsThisModelAHeli(hash) or IsThisModelAPlane(hash) then
        proximity = config.voice_proximity_vehicle
      end
    elseif tvRP.isInside() then
      proximity = config.voice_proximity_inside
    end
...
--function for stopping any camera on
function tvRP.stopAnyCamera()
...
function tvRP.notifyScreen(title,text)
	Citizen.CreateThread(function()
...
function tvRP.CreateNPC(hash,x,y,z,h)
  RequestModel(hash)
...
function tvRP.freeezeEnity(entity,bool)
  FreezeEntityPosition(entity, bool)
end
...
function tvRP.getWaypointCoords()

  local waypointBlip = GetFirstBlipInfoId(8)
  local x,y = table.unpack(Citizen.InvokeNative(0xFA7C7F0AADF25D09, waypointBlip, Citizen.ResultAsVector())) 

  
  if not IsWaypointActive() then
    tvRP.notify("Pune waypoint pe harta pentru a continua!")
  end
