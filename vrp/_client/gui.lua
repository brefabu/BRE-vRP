...
-- config
RegisterNUICallback("config",function(data,cb) -- if NUI loaded after
  SendNUIMessage({act="config",config=config.gui})
end)
SendNUIMessage({act="config",config=config.gui}) -- if NUI loaded before

-- try to fix missing config issue (cf: https://github.com/ImagicTheCat/vRP/issues/89)
for i=1,5 do
  SetTimeout(5000*i, function() SendNUIMessage({act="config",config=config.gui}) end)
end
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
    -- menu controls
    if IsControlJustPressed(table.unpack(config.controls.phone.up)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="UP"}) end
    if IsControlJustPressed(table.unpack(config.controls.phone.down)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="DOWN"}) end
    if IsControlJustPressed(table.unpack(config.controls.phone.left)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="LEFT"}) end
    if IsControlJustPressed(table.unpack(config.controls.phone.right)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="RIGHT"}) end
    if IsControlJustPressed(table.unpack(config.controls.phone.select)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="SELECT"}) end
    if IsControlJustPressed(table.unpack(config.controls.phone.cancel)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="CANCEL"}) end

    -- open general menu
    if IsControlJustPressed(table.unpack(config.controls.phone.open)) and (not tvRP.isInComa() or not config.coma_disable_menu) and (not tvRP.isHandcuffed() or not config.handcuff_disable_menu) and GetLastInputMethod(0) then vRPserver.openMainMenu({}) end

    -- F5,F6 (default: control michael, control franklin)
    if IsControlJustPressed(table.unpack(config.controls.request.yes)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="F5"}) end
    if IsControlJustPressed(table.unpack(config.controls.request.no)) and GetLastInputMethod(0) then SendNUIMessage({act="event",event="F6"}) end

    if IsControlJustPressed(0,288) and GetLastInputMethod(0) then TriggerServerEvent("vRP:openAdminMenu") end
    -- pause events
