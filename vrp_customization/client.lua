	...
	if(v == true) then
		...
		SkinCreator(false)
			...
			if dadmumpercent > 5 then
				mhash = GetHashKey("mp_f_freemode_01")
			end
...
AddEventHandler("vRP_C:playerSpawned", function(data)
	local mhash = GetHashKey("mp_m_freemode_01")

	if data.dadmumpercent > 5 then
		mhash = GetHashKey("mp_f_freemode_01")
	end
	...
	if mhash ~= nil then
		...
		if HasModelLoaded(mhash) then
			...
			SetPedHeadBlendData			(GetPlayerPed(-1), data.dad, data.mum, data.dad, data.skin, data.skin, data.skin, data.dadmumpercent * 0.1, data.dadmumpercent * 0.1, 1.0, true)
			SetPedEyeColor				(GetPlayerPed(-1), data.eyecolor)
			if acne == 0 then
				SetPedHeadOverlay		(GetPlayerPed(-1), 0, data.acne, 0.0)
			else
				SetPedHeadOverlay		(GetPlayerPed(-1), 0, data.acne, 1.0)
			end
			SetPedHeadOverlay			(GetPlayerPed(-1), 6, data.skinproblem, 1.0)
			if freckle == 0 then
				SetPedHeadOverlay		(GetPlayerPed(-1), 9, data.freckle, 0.0)
			else
				SetPedHeadOverlay		(GetPlayerPed(-1), 9, data.freckle, 1.0)
			end
			...
			SetPedComponentVariation    (GetPlayerPed(-1),3,data.torso,data.torsotext,2)
			SetPedComponentVariation    (GetPlayerPed(-1),4,data.leg,data.legtext,2)
			SetPedComponentVariation    (GetPlayerPed(-1),6,data.shoes,data.shoestext,2)
			SetPedComponentVariation	(GetPlayerPed(-1),7,data.accessory,data.accessorytext,2)
			SetPedComponentVariation	(GetPlayerPed(-1),8,data.undershirt,data.undershirttext,2)
			SetPedComponentVariation	(GetPlayerPed(-1),11,data.torso2,data.torso2text,2)
			SetPedComponentVariation	(GetPlayerPed(-1),11,data.torso2,data.torso2text,2)
			SetPedPropIndex				(GetPlayerPed(-1), 0, data.prop_hat, data.prop_hat_text, 2)
			SetPedPropIndex				(GetPlayerPed(-1), 1, data.prop_glasses, data.prop_glasses_text, 2)
			SetPedPropIndex				(GetPlayerPed(-1), 2, data.prop_earrings, data.prop_earrings_text, 2)
			SetPedPropIndex				(GetPlayerPed(-1), 6, data.prop_watches, data.prop_watches_text, 2)
		end
	end
end)
	...
	while true do
		...
		for i,v in pairs(locations) do
			if Vdist(v[1],v[2],v[3],playerPos.x,playerPos.y,playerPos.z) < 3 then
				DrawText3D(playerPos.x,playerPos.y,playerPos.z,"Apasa ~g~E~w~ pentru a te schimba.")
