...
Citizen.CreateThread(function()
	while true do
		...
		local currentVeh = (GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))
		for i = 1, #blacklistedVehicles do
			if GetHashKey(blacklistedVehicles[i]) == currentVeh then
		...
		if vehicle and engine then
			...
			for i = 1, #Vehicles do
				if Vehicles[i].plate == plate then
					...
					local found = false
					local currentVeh = (GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))
					for j = 1, #blacklistedVehicles do
						if GetHashKey(blacklistedVehicles[j]) == currentVeh then
							InBlacklistedVehicle = true
							found = true
							break
						end
...
Citizen.CreateThread(function()
	...
	local blip = AddBlipForCoord(2047.9633789063,3995.2231445313,33.16223526001)
